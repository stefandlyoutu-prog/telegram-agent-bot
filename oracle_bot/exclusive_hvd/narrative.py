...
_HVD_EPIGRAPHS = {
    "С чего начинается эта книга": "Познай себя — и ты познаешь весь мир.",
    "Твоя кармическая типология": "Характер — это судьба, прожитая изнутри.",
    "Твоё тело и темперамент": "Тело первым знает правду о нашей жизни.",
    "Твой характер и сердце": "Сердце имеет доводы, которых не знает разум.",
    "Как ты думаешь": "Мы становимся тем, о чём думаем чаще всего.",
    "Карта твоей энергии: семь центров": "Где течёт твоя энергия — там и твоя жизнь.",
    "Чего ты хочешь и что можешь": "Желание — это половина пути, действие — вторая.",
    "Твой путь по возрастам": "У каждого возраста своя мудрость и свой урок.",
    "Задачи твоей души: прошлое, настоящее, род": "Мы продолжаем то, что начали задолго до нас.",
    "Как жить в своём плюсе": "Сила не в том, чтобы не уставать, а в том, чтобы возвращаться к себе.",
    "Напутствие лично тебе": "Стань тем, кто ты есть на самом деле.",
    "Паспорт ХВД": "За каждой цифрой здесь — живой человек.",
}


def book_for_pdf(profile: HVDProfile, sections: list[Section]) -> str:
    """Богатая книга для PDF: кодированные главы (обложка + оглавление + главы + паспорт)."""
    from oracle_bot.book_pdf import encode_book

    chapters: list[tuple[str, str, str]] = [
        (s.title, s.body, _HVD_EPIGRAPHS.get(s.title, "")) for s in sections
    ]
    chapters.append(
        ("Паспорт ХВД", _passport_plain(profile), _HVD_EPIGRAPHS["Паспорт ХВД"])
    )
    return encode_book(
        title="ХВД",
