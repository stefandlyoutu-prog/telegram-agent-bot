...
_ULTRA_EPIGRAPHS = {
    # LLM-главы (narrative)
    "Кто ты": "Ты — единственный в своём роде замысел.",
    "Твои таланты и дары рода": "Дар, которым делишься, растёт вдвойне.",
    "Твоё предназначение": "Найди, зачем ты здесь, — и жизнь станет лёгкой.",
    "Деньги и реализация": "Деньги приходят туда, где ты на своём месте.",
    "Любовь и отношения": "Любовь начинается с того, как ты относишься к себе.",
    "Род и родители": "Мы стоим на плечах тех, кто был до нас.",
    "Душа: прошлое и продолжение": "Душа помнит больше, чем кажется уму.",
    "Здоровье и энергия": "Тело — это дом, в котором живёт твоя судьба.",
    "Напутствие и год впереди": "Лучшее время начать — сейчас.",
    "Дополнение: твой энергетический портрет": "Энергия не лжёт — она просто течёт.",
    # Корпусные главы (фолбэк-ассемблер)
    "Личные качества": "Ты — единственный в своём роде замысел.",
    "Таланты от Бога": "В каждом из нас спрятан дар, ждущий своего часа.",
    "Таланты по линии матери": "Сила матери живёт в нас тише, но глубже.",
    "Таланты по линии отца": "От отца мы получаем опору и направление.",
    "Предназначение": "Найди, зачем ты здесь, — и жизнь станет лёгкой.",
    "Деньги": "Деньги приходят туда, где ты на своём месте.",
    "Программы (кармические каналы)": "Понять программу — значит перестать ей подчиняться.",
    "Сексуальность": "Тело говорит на языке, который не умеет лгать.",
    "Прошлая жизнь": "Душа помнит больше, чем кажется уму.",
    "Родители": "Мы стоим на плечах тех, кто был до нас.",
    "Родовые программы (мужская линия)": "По мужской линии течёт сила рода.",
    "Родовые программы (женская линия)": "По женской линии течёт мудрость рода.",
    "Обиды на родителей": "Прощение освобождает прежде всего того, кто прощает.",
    "Дети": "В детях мы продолжаемся дальше, чем думаем.",
    "Отношения": "Любовь начинается с того, как ты относишься к себе.",
    "Здоровье — личные рекомендации": "Тело — это дом, в котором живёт твоя судьба.",
    "Руководство по жизни": "Лучший компас — это честность с собой.",
    "Прогноз на год": "Будущее любит тех, кто к нему готов.",
    "Дополнение: Хронально-Векторная Диагностика": "Энергия не лжёт — она просто течёт.",
    "Справка: ваши числа матрицы": "За каждой цифрой здесь — живой человек.",
}


def book_for_pdf(profile: MatrixProfile, sections: list[BookSection]) -> str:
    """Богатая книга Ultra Plus для PDF: обложка + оглавление + главы с эпиграфами."""
    from oracle_bot.book_pdf import encode_book

    chapters: list[tuple[str, str, str]] = []
    for s in sections:
        title = getattr(s, "title", "").strip()
        body = getattr(s, "body", "").strip()
        if not body:
            continue
        # пропускаем служебную «титульную» секцию (имя/дата) — её роль берёт обложка
        if title.startswith(profile.name) or "Ultra Plus" in body[:60]:
            continue
        chapters.append((title, body, _ULTRA_EPIGRAPHS.get(title, "")))
    return encode_book(
        title="Книга о тебе",
        subtitle="Персональный разбор по Матрице Судьбы · Ultra Plus",
