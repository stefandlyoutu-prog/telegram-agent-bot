...
Обновлено: 2026-08-04. Агент делает всё, что можно без вас; ниже — только то, где нужен человек.
...
## Новое: VPN-бот (см. `docs/VPN_BOT_SPEC.md`)

Код бота и клиент Marzban уже готовы (`vpn_bot/`). Без тебя не получится:
покупка VPS требует твоей карты/оплаты, домен/капчи хостера — тоже руками.

1. **Арендовать VPS в Европе** (Нидерланды/Франкфурт/Хельсинки — ближе к РФ):
   - Рекомендую **Aeza** (https://aeza.net) — принимает карты РФ и СБП, тариф
     Shared ~1 vCPU/2GB/30GB NVMe за ~€4.94–650₽/мес, локация Amsterdam/Frankfurt/Helsinki,
     ОС **Ubuntu 24.04**.
   - Альтернатива: VDSina (https://vdsina.ru, от ~200₽/мес, тоже карты РФ).
   - При заказе: ОС Ubuntu 24.04 LTS, минимум 1 vCPU / 2 ГБ RAM / 20+ ГБ диска.
   - После оплаты получишь **IP, root-логин, пароль** (или SSH-ключ) на почту/в ЛК.
2. **Написать агенту IP + пароль/ключ от VPS** (или дать доступ иначе) — тогда я сам
   зайду по SSH и прогоню `scripts/setup_marzban_vps.sh` (установка Marzban + Xray Reality).
   Либо сделай это сам по инструкции в начале файла скрипта.
3. Создать логин/пароль панели: `marzban cli admin create --sudo` (на самом VPS).
4. Завести нового бота у @BotFather (`/newbot`) → токен → скажи агенту, он впишет
   в Render (`VPN_BOT_TOKEN`, `MARZBAN_BASE_URL/USERNAME/PASSWORD`) и задеплоит.
5. (Опционально, но желательно) Робокасса — можно тот же магазин, что у Оракула,
   либо завести отдельный на https://www.robokassa.ru/ (если хочешь разделять учёт).
